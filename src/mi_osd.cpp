...
*/
  ...
  for(auto it = cache_.jacobians.begin(); it != cache_.jacobians.end(); ++it)
  {
    ...
    std::cout << "temp Full Jacobian size is: " << tempFullJacobian.rows() << ", " << tempFullJacobian.cols()
              << std::endl;
    if(useLinearJacobian_())
    {
  ...
  std::cout << "The OSD inverse mass matrix has row: " << cache_.lambdaMatrixInv.rows()
            << ", col: " << cache_.lambdaMatrixInv.cols() << ", rank: " << lu_decomp_lambdaInv.rank() << std::endl;
  ...
  for(int ii = 0; ii < getEeNum(); ii++)
  {
    ...
    cache_.rhoTwo.segment(ii * getJacobianDim(), getJacobianDim()) =
        cache_.dcJacobianInvs[ii].transpose() * getFD()->C();
  ...
  for(int ii = 0; ii < getEeNum(); ii++)
  {
    for(int jj = 0; jj < getEeNum(); jj++)
    {
      ...
      cache_.crossLambdaMatrix.block(ii * getJacobianDim(), jj * getJacobianDim(), getJacobianDim(), getJacobianDim()) =
  ...
  cache_.rhoOne = -cache_.lambdaMatrix * cache_.osdJacobianDot * jointVel;
  ...
  */
  ...
  for(auto it = cache_.jacobians.begin(); it != cache_.jacobians.end(); ++it)
  {
    ...
    std::cout << "Test " << it->first << " Jacobian multiplies DC Jacobian inverse: " << std::endl
              << cache_.osdJacobian.block(index * getJacobianDim(), 0, getJacobianDim(), getDof())
    ...
    std::cout << "The OSD dynamics of " << it->first << " index-" << index << " is: " << std::endl
              << cache_.lambdaMatrix.block(index * getJacobianDim(), index * getJacobianDim(), getJacobianDim(),
                                           getJacobianDim())
  ...
  std::cout << "The gravity is: " << getRobot().mbc().gravity.transpose() * getRobot().mass()
            ...
            << " Right foot F sensor: " << getRobot().forceSensor("RightFootForceSensor").force() << std::endl;
  ...
  std::cout << "The OSD Vel is: " << std::endl << cache_.osdVel << std::endl;
