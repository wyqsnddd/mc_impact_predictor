...
void mi_impactPredictor::run()
{
  ...
  std::cout << "The velocity of " << getImpactBody_()
            << " is: " << getDartRobot()->getBodyNode(getImpactBody_())->getSpatialVelocity() << std::endl;
  // std::cout<<"q_dd is: "<<alphaD.transpose()<<std::endl;
  // std::cout<<" The Jacobian is: "<<getOsd_()->getJacobian(getImpactBody_())<<std::endl;

  cache_.eeVelJump = -(getCoeRes_() + 1) * getOsd_()->getJacobian(getImpactBody_())
