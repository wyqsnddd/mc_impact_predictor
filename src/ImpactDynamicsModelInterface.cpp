...
*/
void TwoDimModelBridge::computeGradient(const Eigen::Vector3d & impactNormal, Eigen::Vector3d & jumpDirection)
{
  impactNormal_ = impactNormal;
  // (0) Compute the whole-body inertia and average velocity
  Eigen::Matrix6d centroidalInertia;
  ...
  // gradientApproximation_(contactVelGrids, contactVelGrids, comVxJumpGrids, comVyJumpGrids,
  // bridgeParams_.gradientParams);
                                               ...
                                               GradientApproximationParams & params)
{
  ...
  gsl_fit_linear(contactVxGrids, 1, comVxGrids.dataVec_.data(), 1,
                 getTwoDimModelBridgeParams().gradientParams.numCaurseGrid, &params.gradientX.c0, &params.gradientX.c1,
                 &params.gradientX.cov00, &params.gradientX.cov01, &params.gradientX.cov11, &params.gradientX.sumsq);
  ...
  {
    std::cout<<comVxGrids[ii]<<" =  "<< params.gradientX.c1<<" * "<<contactVxGrids[ii]<< " + "<<params.gradientX.c0<<
  std::endl;;
  }
  std::cout<<"-------------------"<<std::endl;
  */
  ...
  gsl_fit_linear(contactVyGrids, 1, comVyGrids.dataVec_.data(), 1,
                 getTwoDimModelBridgeParams().gradientParams.numCaurseGrid, &params.gradientY.c0, &params.gradientY.c1,
                 &params.gradientY.cov00, &params.gradientY.cov01, &params.gradientY.cov11, &params.gradientY.sumsq);
  ...
  for(unsigned ii = 0; ii<caurseContactVelocityGrids_.size(); ii++)
  {
    std::cout<<comVyGrids[ii]<<" =  "<< params.gradientY.c1<<" * "<<contactVyGrids[ii]<< " + "<<params.gradientY.c0<<
  std::endl;;
  }
  std::cout<<"-------------------"<<std::endl;
  */
                                                     ...
                                                     fittingParams & params)
{
  ...
  const size_t dim = 1; // linear fit
  gsl_matrix *contactVelData, *cov;

  // comVel = contactVel(i) * coe(0)

  // gsl_vector *vx,*vy,*vz;
  gsl_vector *comVel, *coe;

  contactVelData = gsl_matrix_alloc(n, dim);

  comVel = gsl_vector_alloc(n);
  ...
  for(size_t i = 0; i < n; ++i)
  {
    ...
    gsl_matrix_set(contactVelData, i, 0, contactVelGrids[i]);

    // Set the contact velocity
    gsl_vector_set(comVel, i, comVelGrids[i]);
  }

  // Allocate the work space
  gsl_multifit_robust_workspace * workSpace =
      gsl_multifit_robust_alloc(gsl_multifit_robust_bisquare, contactVelData->size1, contactVelData->size2);

  // Perform the fitting
  gsl_multifit_robust(contactVelData, comVel, coe, cov, workSpace);
...
void TwoDimModelBridge::paramUpdatePushWall_(const Eigen::Vector3d & impactLinearVel)
{
  ...
  // (1) Robot
  piParams_.batParams.com =
      RoboticsUtils::vectorOnSurface(getRobot()->com(), Eigen::Vector3d::UnitZ()).block<2, 1>(0, 0);
  ...
  piParams_.batParams.preVel =
      RoboticsUtils::vectorOnSurface(impactLinearVel, Eigen::Vector3d::UnitZ()).block<2, 1>(0, 0);
...
void writeEntry_(std::ofstream & myfile, const std::string & name, const size_t & size, const double * input)
{
...
std::string TwoDimModelBridge::saveFidModelGradientData()
{
  ...
  name.append(std::to_string(im.x()) + "_" + std::to_string(im.y()) + "_" + std::to_string(im.z())
              + "_FIDGradient.yaml");
  myfile.open(name);
